...
import { comprobarCodigoPromocional, listarProductos } from '../../../../../controllers/suscriptores/SuscriptoresController';
import ClickAwayListener from 'react-click-away-listener';

const InformacionSuscripcion = ({ suscripciones, setSuscripciones }) => {
    ...
    const handleProductoChange = (index, productoSeleccionado) => {
        if (productoSeleccionado) {
            ...
            nuevasSuscripciones[index] = {
                ...
                id_codigo_promocional: idCodigoPromocional || productoSeleccionado.id_codigo_promocional || 0,
                id_vendedor: datosVendedor?.id_usuario_vendedor || nuevasSuscripciones[index].id_vendedor,
                id_suscripcion_vendedor: datosVendedor?.id_suscripcion_vendedor || nuevasSuscripciones[index].id_suscripcion_vendedor,
                vendedor: datosVendedor?.nombre_vendedor || nuevasSuscripciones[index].vendedor
            };
            setSuscripciones(nuevasSuscripciones);
        }
    };
    ...
    const onClickSuggestion = (item, index) => {
        handleSuscripcionChange(index, 'vendedor', `${item.usuario} - ${item.nombres}`);
        ...
        <div className='w-full border-2 border-gray-300 rounded-lg mt-5 relative bg-white shadow-sm'>
            ...
            <div className='p-6 pt-8'>
                ...
                <div className='mb-6 p-4 bg-blue-50 rounded-lg border border-blue-200'>
                    <h3 className='text-sm font-semibold text-gray-700 mb-3'>Buscar Productos por Código Promocional</h3>
                        ...
                        <div className='mt-3'>
                            ...
                            {datosVendedor && (
                                <div className='text-xs text-blue-700 bg-blue-50 p-2 rounded border border-blue-200'>
                                    <span className='font-semibold'>Vendedor asignado:</span> {datosVendedor.nombre_vendedor}
                                    <span className='ml-2 text-gray-600'>(ID: {datosVendedor.id_usuario_vendedor})</span>
                                </div>
                            )}
                        </div>
                    )}
                ...
                {suscripciones && suscripciones.length > 0 ? (
                            ...
                            <div key={index} className='border border-gray-300 rounded-lg p-4 bg-gray-50 relative'>
                                ...
                                <div className='grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4'>
                                    ...
                                    <div className='flex flex-col'>
                                        ...
                                        <input
                                            value={item.titulo || ''}
                                            onChange={(e) => handleSuscripcionChange(index, 'titulo', e.target.value)}
                                            type='text'
                                    ...
                                    <div className='flex flex-col'>
                                        ...
                                        <input
                                            value={item.precio || ''}
                                            onChange={(e) => handleSuscripcionChange(index, 'precio', e.target.value)}
                                            type='number'
                                            step='0.01'
                                            className='border border-gray-300 rounded-md px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-greenVE-500 bg-gray-100'
                                            placeholder='0.00'
                                    ...
                                    <div className='flex flex-col'>
                                        ...
                                        <input
                                            ...
                                            onChange={(e) => handleSuscripcionChange(index, 'fecha_inicio', e.target.value)}
                                            type='date'
                                            className='border border-gray-300 rounded-md px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-greenVE-500'
                                        />
                                    </div>
                                    ...
                                    <div className='flex flex-col'>
                                        ...
                                        <input
                                            ...
                                            onChange={(e) => handleSuscripcionChange(index, 'fecha_fin', e.target.value)}
                                            type='date'
                                            className='border border-gray-300 rounded-md px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-greenVE-500'
                                        />
                                    </div>
                                    ...
                                    <div className='flex flex-col'>
                                        ...
                                        <select
                                            ...
                                            onChange={(e) => handleSuscripcionChange(index, 'id_estado_pago', e.target.value)}
                                            className='border border-gray-300 rounded-md px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-greenVE-500'
                                        >
                                    ...
                                    <div className='flex flex-col'>
                                        ...
                                        <select
                                            ...
                                            onChange={(e) => handleSuscripcionChange(index, 'id_canal', e.target.value)}
                                            className='border border-gray-300 rounded-md px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-greenVE-500'
                                        >
                                            {canales && canales.map((canal) => (
                                                <option key={canal.id_tbl_tipo_canal} value={canal.id_tbl_tipo_canal}>
                                                    {canal.nombre}
                                                </option>
                                            ))}
                                        </select>
                                    ...
                                    <div className='flex flex-col relative md:col-span-2'>
                                        <label className='text-xs font-medium text-gray-700 mb-1'>
                                            Vendedor {datosVendedor && <span className='text-blue-600'>(Asignado automáticamente)</span>}
                                        </label>
                                        <input
                                            value={item.vendedor || ''}
                                            onChange={(e) => handleVendedorInputChange(e.target.value, index)}
                                            type='text'
                                            className={`border border-gray-300 rounded-md px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-greenVE-500 ${
                                        ...
                                        />
                                        {suggestion && selectedIndex === index && !datosVendedor && (
                                                ...
                                                <div className="absolute top-full mt-1 max-h-64 w-full bg-white z-50 shadow-lg border border-gray-300 rounded-md overflow-y-auto">
                                                    {suggestion.map((vendedor, key) => (
                                                        <div
                                                            ...
                                                            onClick={() => onClickSuggestion(vendedor, index)}
                                                        >
                                                            <div className="flex justify-between items-center">
                                                                <span className="text-sm">
                                                                    ID: {vendedor.usuario} | CI: {vendedor.ci_ruc} | {vendedor.nombres}
                                                                </span>
                                    ...
                                    <div className='flex flex-col md:col-span-3'>
                                        ...
                                        <textarea
                                            ...
                                            onChange={(e) => handleSuscripcionChange(index, 'observacion', e.target.value)}
                                            className='border border-gray-300 rounded-md px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-greenVE-500'
                                    ...
                                    {(item.id_producto || item.id_prod_suscripcion || item.id_lista_precio_producto || item.id_suscripcion_vendedor) && (
                                        <div className='md:col-span-3 p-3 bg-gray-100 rounded border border-gray-300'>
                                            <p className='text-xs text-gray-600 font-semibold mb-1'>IDs (automáticos):</p>
                                            <div className='grid grid-cols-2 lg:grid-cols-4 gap-2 text-xs text-gray-700'>
                                                {item.id_producto && <span>ID Producto: <strong>{item.id_producto}</strong></span>}
                                                {item.id_prod_suscripcion && <span>ID Prod. Susc.: <strong>{item.id_prod_suscripcion}</strong></span>}
                                                {item.id_lista_precio_producto && <span>ID Lista Precio: <strong>{item.id_lista_precio_producto}</strong></span>}
                                                {item.id_suscripcion_vendedor && <span className='text-blue-700'>ID Susc. Vendedor: <strong>{item.id_suscripcion_vendedor}</strong></span>}
                                            </div>
                                        </div>
                                    )}
                                </div>
                    ...
                    <div className='text-center py-12 text-gray-500 bg-gray-50 rounded-lg border-2 border-dashed border-gray-300'>
                        ...
                        <p className='text-sm font-medium'>No hay suscripciones agregadas</p>
                        <p className='text-xs text-gray-400 mt-1'>Busca productos con un código promocional y luego agrega una suscripción</p>
                    </div>
                )}
                ...
                {suscripciones.length > 0 && (
                    <div className='mt-6 p-3 bg-yellow-50 border-l-4 border-yellow-500 rounded-r-md'>
                        <p className='text-xs text-yellow-800'>
                            <span className='font-semibold'>Importante:</span> Los IDs de producto y vendedor se asignan automáticamente al buscar con un código promocional.
                        </p>
                    </div>
                )}
            </div>
